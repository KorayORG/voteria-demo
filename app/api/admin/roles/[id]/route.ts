...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.code !== undefined) updates.code = body.code?.toString().toLowerCase().slice(0,40) || undefined
    ...
    await db.collection('audit_logs').insertOne({ action:'ROLE_UPDATED', actorId: ctx.userId || 'system', actorName:'Sistem', actorIdentityNumber, entity:'Role', entityId: new ObjectId(id), targetId: new ObjectId(id), targetName: updates.name || res.value.name, meta: updates, createdAt:new Date() })
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!authz.ok) return NextResponse.json({ error: authz.error }, { status: 403 })
    ...
    const db = client.db('cafeteria')
    const col = db.collection('roles')
    const role = await col.findOne({ _id: new ObjectId(id) })
    ...
    await db.collection('audit_logs').insertOne({ action:'ROLE_DELETED', actorId: ctx.userId || 'system', actorName:'Sistem', actorIdentityNumber, entity:'Role', entityId: new ObjectId(id), targetId: new ObjectId(id), targetName: role.name, meta:{ name: role.name }, createdAt:new Date() })
    return NextResponse.json({ success: true })
  } catch (e) {
